...
filename = 'presidenciales/total_anuncios.csv'
countries = ['PE']
page_limit = 50
...
def read_id_file():
    data = pd.read_csv("ids_presidenciales.csv", dtype=str)
...
if __name__ == '__main__':
    ...
    #anuncios[anuncios['electorales'] == True].to_csv('presidenciales/activos_elecciones.csv')

    #anuncios_activos = pd.read_csv('presidenciales/activos_elecciones.csv')
    #anuncios_q = anuncios_activos[['id', 'page_id']].groupby(['page_id']).count().unstack()
    #anuncios_q.to_csv('presidenciales/conteo_anuncios.csv')

    #anuncios_spend = anuncios_activos[['page_id', 'currency', 'lower_bound', 'upper_bound']].groupby(['page_id', 'currency']).sum().unstack()
    #anuncios_spend.to_csv('presidenciales/gasto_anuncios.csv')
    #anuncios['silencio1'] = anuncios.apply(lambda x: silencio(x['ad_delivery_start_time'], x['ad_delivery_stop_time']),
                                           #axis=1)
    ...
    #anuncios_prohibidos[['id', 'ad_creation_time', 'ad_delivery_start_time', 'ad_delivery_stop_time', 'page_id', 'page_name', 'ad_creative_bodies']].to_csv('presidenciales/anuncios_silencio_electoral.csv')

    ##working with regions
    anuncios['min_impression'] = anuncios.apply(lambda x: impressions(x['impressions'], 'lower_bound'), axis=1)
    print(anuncios[['page_id', 'min_impression']])
